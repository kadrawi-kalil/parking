...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (networkData) {
      ...
      clientplacefiltered:this.state.places
    });
  ...
  uploadPlace = (description, date, time, nbHour) => {
    ...
    ipfs.add(this.state.buffer, (error, result) => {
      ...
      this.state.decentragram.methods
        .uploadPlace(result[0].hash, description, date, time, nbHour)
  ...
  updateInput(input) {
    console.log(input, this.state.places);
    const filtered = this.state.places.filter((place) => {
      return place.description.toLowerCase().includes(input.toLowerCase());
    });
    this.setState({ input: input, clientplacefiltered: filtered });
  }
  constructor(props) {
    super(props);
    this.state = {
          ...
          <Main
            places={
              this.state.owner === this.state.account
                ? this.state.clientplacefiltered
                : this.state.clientplace
            }
            captureFile={this.captureFile}
            uploadPlace={this.uploadPlace}
            tipPlaceOwner={this.tipPlaceOwner}
            owner={this.state.owner === this.state.account}
